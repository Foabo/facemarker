...
# 图片数据文件夹
# INPUT_DATA = r'/Users/inf/project/face/facemarker_srv/POSE_160_RENAME'

INPUT_DATA = r'/Users/inf/project/face/facemarker_srv/pictures_resturct'
...
def create_image_lists():
    ...
    while len(matched_result) < 68:
        for sub_dir in sub_dirs[1:]:
            ...
            name, number = base_name1.split("_", 1)
            ...
            name, number = base_name2.split("_", 1)
            number = number.rstrip(".jpg")
            number = str(number).strip("000")
            base_name2 = number
...
def main():
    ...
    file = open(r'/Users/inf/project/face/facemarker_srv/scholar_pairs.txt', 'w')
    ...
    for i in range(epoch):
        j = 0
        print("=============================================第" + str(i) + '次, 相同的')
        for pair in result1[i*mul_num:i*mul_num+mul_num]:
            j = j + 1
            print(str(j) + ': ' + pair)
            file.write(pair + '\n')

        print("=============================================第" + str(i) + '次, 不同的')
        for pair in result2[i*mul_num:i*mul_num+mul_num]:
            j = j + 1
            print(str(j) + ': ' + pair)
            file.write(pair + '\n')
    file.close()
def pairs():
    result = []
    with open('/Users/inf/project/face/facemarker_srv/POSE_pairs2.txt', 'r') as f:
        for line in f.readlines()[1:]:
            # print(line)
            line_list = line.split()
            # print(line_list)
            name, number = line_list[1].split("_", 1)
            number = number.rstrip(".jpg")
            number = str(number).strip("000")
            print(number)
            line_list[1]=number
